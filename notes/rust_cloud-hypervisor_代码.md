...
- [code walk in single picture](#code-walk-in-single-picture)
- [编译](#编译)
  - [build时可选的feature列表](#build时可选的feature列表)
- [REST API和CLI](#rest-api和cli)
  - [先用cli创建一个empty的实例, 默认1vCPU, 512M内存.](#先用cli创建一个empty的实例-默认1vcpu-512m内存)
  - [随后用REST API来创建vm](#随后用rest-api来创建vm)
  - [然后boot这个实例](#然后boot这个实例)
  - [其他命令](#其他命令)
  - [cli和REST的关系](#cli和rest的关系)
  - [http路由](#http路由)
  - [内部channel](#内部channel)
- [代码梳理 main](#代码梳理-main)
  - [hypervisor的抽象](#hypervisor的抽象)
    - [vm抽象基本上是基于kvm api的](#vm抽象基本上是基于kvm-api的)
    - [vmops](#vmops)
- [cli create vm代码流程](#cli-create-vm代码流程)
- [REST API流程实例](#rest-api流程实例)
  - [用户发REST API](#用户发rest-api)
  - [这个VMM对应的http server响应请求](#这个vmm对应的http-server响应请求)
    - [从http的raw data里(json格式)解析VmConfig](#从http的raw-data里json格式解析vmconfig)
    - [vm_create使用内部channel向VMM的API发送请求](#vm_create使用内部channel向vmm的api发送请求)
  - [内部channel处理请求](#内部channel处理请求)
    - [处理这次的VmCreate](#处理这次的vmcreate)
  - [返回response](#返回response)
- [impl Vmm](#impl-vmm)
  - [x86_64和aarch64的mem layout](#x86_64和aarch64的mem-layout)
  - [vm_boot](#vm_boot)
  - [DeviceManager](#devicemanager)
    - [DeviceManager结构体](#devicemanager结构体)
    - [DeviceManager方法](#devicemanager方法)
      - [DeviceNode包括id, 资源, 层级, pci信息](#devicenode包括id-资源-层级-pci信息)
...
- [virtio设备](#virtio设备)
  - [virtionet](#virtionet)
    - [Net结构体](#net结构体)
    - [impl VirtioDevice for Net](#impl-virtiodevice-for-net)
      - [activate](#activate)
      - [NetQueuePair](#netqueuepair)
    - [其他trait](#其他trait)
  - [virtio block](#virtio-block)
- [IO Bus和MMIO Bus](#io-bus和mmio-bus)
  - [Bus方法](#bus方法)
- [中断](#中断)
  - [interrupt group](#interrupt-group)
  - [中断控制器](#中断控制器)
    - [msi中断控制器](#msi中断控制器)
